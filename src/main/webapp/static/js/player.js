        ...
        if (totalPlaylist.length > 8){
            ...
            for (let i = 0; i < 8; i++) {
                ...
                for (let j = 0; j < songs.length; j++) {
                    //获得歌曲哈希值
                    var hash = songs[j]['hash']

                    //发送请求获得详细歌曲信息
                    var musicInfo = getMusicInfo(hash)

                    //获得 文件头信息,用户替换url地址中的 {size}
                    var fileHead = musicInfo['fileHead']

                    //获得歌曲id
                    var audio_id = musicInfo['audio_id']

                    //获得歌曲url
                    var url = musicInfo['url']

                    //歌曲名
                    var fileName = musicInfo['fileName']

                    //歌手名
                    var author_name = musicInfo['author_name']

                    //歌手写真
                    var imgUrl = musicInfo['imgUrl']
                    imgUrl = imgUrl.replace(/{size}/, fileHead)

                    //歌曲专辑图片
                    var album_img = musicInfo['album_img']
                    album_img = album_img.replace(/{size}/, fileHead)

                    //创建json
                    var musicInfoJson = {
                        "id": audio_id, "title": fileName,
                        "singer": author_name, "songUrl": url,
                        "imageUrl": imgUrl,
                        "album_img": album_img
                    }

                    //添加歌曲进入列表
                    songArray.push(musicInfoJson)
                }
            }
        }else {
            //获得当前歌单
 ...
 * @param hash 歌曲哈希值
 */
function getMusicInfo(hash) {
    var musicInfo = ""
    $.ajax({
        url: "music/musicInfo.action",
        async: false,
        type: "get",
        data: {
            cmd: "playInfo",
...
class PlayerCreator {
    ...
    renderSongStyle() {
        ...
        this.render_doms.blur.css('background-image', 'url("' + album_img + '")');
        this.render_doms.album_img.css('background-image', 'url("' + album_img + '")');

        //切换列表中的item的类名 play
        this.song_list.find('.music__list__item').eq(this.song_index).addClass('play').siblings().removeClass(
            'play');
    }
    ...
    //歌曲时长
    songTime() {
        let totalMinute = parseInt(this.audio.duration / 60) < 10 ? "0" + parseInt(this.audio.duration / 60) :
            parseInt(this.audio.duration / 60);
        let totalSecond = parseInt(this.audio.duration % 60) < 10 ? "0" + parseInt(this.audio.duration % 60) :
            parseInt(this.audio.duration % 60);
        ...
        ele.mousedown((e) => {
            ...
            $(document).mousemove((e) => {
                if (flag === true) {
                    ...
                    let _distance = Math.max(0, Math.min(_left, father.outerWidth(false) - ele
                        .outerWidth(false)))
