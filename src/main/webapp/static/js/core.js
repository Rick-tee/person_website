 ...
 * 显示或隐藏悄悄话
 */
function hideOrShowTalk(){
    //获得用户信息
    var user =  JSONStrToJSON($.cookie("user"));

    //判断用户心愿单是否存在,如果内容不存在，则隐藏
    if (user.leftWish == null){
        //隐藏左边
        $("#one").hide()
...
function getRandomPic(){
    return "https://www.ylesb.com/RandomImages/" + Math.ceil(Math.random() * 3000) + ".png";
...
function setRandomPic(){
    //轮播图部分
    for (let i = 1; i < 7; i++) {
        $('.getDyPic' + i).attr("src",getRandomPic())
    }

    //我的视频，相册，歌单
    $('#myVideo').attr("src",getRandomPic())
    $('#myMusic').attr("src",getRandomPic())
    $('#myPicture').attr("src",getRandomPic())

    //悄悄话
    $('#want').attr("src",getRandomPic())
    $('#love').attr("src",getRandomPic())
}